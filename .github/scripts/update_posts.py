...
def fetch_latest_posts(feed_url, max_posts):
    ...
    if feed.bozo:
        print(
            f"Warning: Feed parsing encountered issues: {feed.bozo_exception}")
            ...
            if releases_response.status_code == 200:
                ...
                for release in releases:
                    release_name = release.get(
                        'name') or release.get('tag_name', 'Release')
...
def generate_activity_markdown(activities):
    ...
    for activity in activities:
        ...
        if activity['type'] == 'commit':
            lines.append(
                f"- **[{activity['repo']}]({activity['repo_url']})**: [{activity['sha']}]({activity['url']}) - {activity['message']} ({formatted_date})")
        elif activity['type'] == 'release':
            lines.append(
                f"- **[{activity['repo']}]({activity['repo_url']})**: Released [{activity['name']}]({activity['url']}) ({formatted_date})")
...
def update_readme(readme_path, posts_markdown, activity_markdown):
    ...
    content = update_readme_section(
        content, POSTS_START_MARKER, POSTS_END_MARKER, posts_markdown)

    # Update activity section
    content = update_readme_section(
        content, ACTIVITY_START_MARKER, ACTIVITY_END_MARKER, activity_markdown)
